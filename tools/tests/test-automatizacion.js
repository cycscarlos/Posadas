  ...
  try {
    ...
    if (habitaciones.length < 5) {
      ...
      for (let i = 0; i < habitacionesFaltantes; i++) {
        ...
        await query(
          `
          INSERT INTO habitaciones (id_habitacion, numero_habitacion, tipo, capacidad, precio, estado, descripcion) 
          VALUES (?, ?, 'Estándar', 2, 100.00, 1, 'Habitación de prueba')
        `,
          [id, `T-${id}`]
        );
      }
  ...
  try {
    ...
    for (const reserva of reservas) {
      await query(
        ...
        (id_cliente, id_habitacion, fecha_entrada, fecha_salida, estado, nombre_cliente, documento, telefono)
        VALUES (999, ?, ?, ?, ?, ?, '12345678', '555-1234')
      `,
        [
...
const mostrarEstadoReservasPrueba = async () => {
  try {
    const reservas = await query(`
      SELECT r.id_reserva, r.id_habitacion, h.numero_habitacion, 
             r.fecha_entrada, r.fecha_salida, r.estado, r.nombre_cliente 
      FROM reservas r
